...
tic
filedata_1=fopen('Caminos_ITALNET_SP.txt');
filewrite=fopen('Matrix_L_P_Red_ITALNET_SP_b.txt','w');
data=textscan(filedata_1,'%f %f %f %f %f %f %f %f','Delimiter',' ');
columnas=7;
fclose(filedata_1);
toc
...
tic
P_P(1:N_path,1:N_path)=0;
for i=1:1:N_path;
    links_in_path=sum(L_P(:,i));
    for p=1:1:N_path;
        if sum((L_P(:,i)==L_P(:,p)).*(L_P(:,i)==1))==links_in_path;
            P_P(i,p)=1;
        end;
    end;
end;
...
fprintf(filewrite,'\n');
fprintf(filewrite,'param P_P:=\n');
for i=1:1:N_path;
    fprintf(filewrite,'[%d,*]',i);
    for j=1:1:N_path;
        if j==N_path && i==N_path;
            fprintf(filewrite,' %d %d;\n',j,P_P(i,j));
        else
            if j==N_path;
                fprintf(filewrite,' %d %d\n',j,P_P(i,j));
            else
                fprintf(filewrite,' %d %d',j,P_P(i,j));
            end;
        end;
    end;
end;
